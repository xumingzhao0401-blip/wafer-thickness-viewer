# -*- coding: utf-8 -*-
"""
Wafer Thickness Viewer (Streamlit) - v4
- NEW: 25-point concentric template (1 + 4 + 8 + 12) with correct point placement (supports rotation).
- Keeps: Fab25 grid-like template, unit conversion, true 3D, PPT export layout, radius column.
"""
...
def _ring_points(r: float, n: int, start_deg: float) -> List[Tuple[float, float]]:
    ang = np.deg2rad(start_deg) + np.linspace(0, 2*np.pi, n, endpoint=False)
...
def generate_pattern_coords(pattern_type, radius_mm, edge_exclude_mm, **kwargs) -> pd.DataFrame:
    ...
    if pattern_type == "十字交叉 (Cross)":
    ...
    elif pattern_type == "同心圆25点 (1+4+8+12)":
        # Requested: 25 points = center(1) + ring1(4) + ring2(8) + ring3(12)
        # Placement: use 3 radii at 1/3, 2/3, 3/3 of effective radius.
        # Angle design (to match typical fab plots and avoid stacking):
        # - ring1 (4): diagonal (45° offset)
        # - ring2 (8): cardinal + diagonal (0° offset, every 45°)
        # - ring3 (12): 15° offset (every 30°)
        rotate = float(kwargs.get("rotate_deg", 0.0))  # user-tunable
        points.append((0.0, 0.0))
        if effective_r > 0:
        ...
        # From provided SZU template image:
        # 25 points = center(1) + cross(2-5) + mid ring(6-13) + outer ring(14-25)
        # Radii are proportional to effective wafer radius (edge exclusion already applied).
        ...
        layout = [(1, 0.0, 0.0)]
        # Cross (small radius): top/right/bottom/left
        ...
        # Mid ring (8 points, slightly "offset" compared to standard 45°)
        mid = {
            6: 60.0,
            7: 15.0,
            8: 345.0,
        ...
        # Outer ring (12 points, every 30°, starting from top=14 and clockwise)
        outer = [
    ...
    if point_id is not None:
        ...
        # point_id sequential (1..N) for concentric templates and others
...
def blind_mode_ui():
    ...
    with col_ctrl:
        ...
        pat_type = st.radio(
            "生成方式",
            ["同心圆25点 (1+4+8+12)", "SZU 25point", "FAB 25点模板 (Fab25)", "十字交叉 (Cross)", "同心圆 (Concentric)", "均匀网格 (Grid)"],
            key="gen_pat",
        )
        ...
        elif pat_type == "同心圆25点 (1+4+8+12)":
            st.caption("中心1点 + 第一圈4点 + 第二圈8点 + 第三圈12点。")
            gen_params["rotate_deg"] = st.slider("整体旋转角度 (deg)", -30.0, 30.0, 0.0, 1.0, key="gen_rot25")

        elif pat_type == "SZU 25point":
            st.caption("中心1点 + 十字4点 + 中圈8点（错位）+ 外圈12点（每30°）。")
            gen_params["rotate_deg"] = st.slider("整体旋转角度 (deg)", -30.0, 30.0, 0.0, 1.0, key="gen_rot_szu25")
    ...
    with col_data:
        ...
        show_cols = ["point_id", "x", "y", "radius", "thickness"]
        show_cols = [c for c in show_cols if c in df_curr.columns]

        column_config = {
            "point_id": st.column_config.NumberColumn("Point ID", disabled=True),
...
def normal_mode_ui():
    ...
    with left:
        ...
        st.text_input("PPT 标题", value=f"Wafer Thickness Report - {ds_name}", key="ppt_title")
    ...
    with right:
        ...
        if err is not None:
            ...
            st.info("若提示 kaleido 缺失：请确认 requirements.txt 里有 kaleido==0.2.1，并在 Streamlit Cloud 清缓存重启。")
        else:
            st.download_button(
                "📤 生成 PPT 并下载",
                data=ppt_bytes,
